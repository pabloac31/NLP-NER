...
class PostagCorpus(object):
    ...
    def read_sequence_list(self,
                                     train_file,
                                     min_sent_len=3,
                                     max_sent_len=100000,
                                     max_nr_sent=100000):
        """
        Reads the data form the Deliverable2 dataset of named entity recognition
        """

        instance_list = self.read_data_instances(train_file,
                                                      min_sent_len,
                                                      max_sent_len,
                                                      max_nr_sent,
                                                      )
    ...
    def read_data_instances(self, file, min_sent_len, max_sent_len, max_nr_sent):
        """
        Reads the data form the Deliverable2 dataset of named entity recognition
        """
        aux = codecs.open(file)
        lines = []
        for line in aux:
            lines.append(line)
        ...
        data = []
        acum = []
        prev_id = lines[0].split(',')[0]
        for line in lines:
            sent_id = line.split(',')[0]
            if sent_id == prev_id: # current sentence
                acum.append(line)
            else: # reading a new sentence
                prev_id = sent_id
                if len(acum) >= min_sent_len:
                    data.append(acum)
                acum = []
        ...
        for d in data:
            ...
            y = []
            for line in d:
                word_list = line.split(',')[1:-1] # to fix situations where there is a ',' in the word
                word = ','.join(word_list)
